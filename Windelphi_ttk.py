...
class Application(Frame):
    ...
    def interface(self):
        ...
        self.inp_find = Entry(frm_search_bar, width=50)
        ...
        self.lbl_trackLength = Label(frm_player, text="00", padding="1 1 5 5")
        ...
        self.frm_image = Frame(self.frm_track, width=300)
        ...
        # Status bar
        self.statusbar = Label(self.root, text="on the way…", relief=SUNKEN, anchor=W)
    ...
    def play(self):
        ...
        if(self.player.playing is None):
            ...
            self.lbl_trackLength.configure(text= str(int(self.player.mp3Length)))
    ...
    def displayTrackInfo(self, info):
        ...
        for k in info.keys():
            kname = str(k + ": ")
            ink = Label(self.frm_track_info, text= kname, bg=self.bgColor, fg =self.fgColor, padding="2 2 5 5")
            if(k in dict.keys()):
                ink.grid(row=dict.get(k).id, column=0)
                inv = Label(self.frm_track_info, text=self.mkString(info[k]), font=dict.get(k).font, bg=self.bgColor, fg =self.fgColor, padding="2 2 5 5")
                inv.grid(row=dict.get(k).id, column=1)
            else:
                ink.grid(row=i, column=0)
                inv = Label(self.frm_track_info, text=self.mkString(info[k]), font=font, bg=self.bgColor, fg =self.fgColor, padding="2 2 5 5")
    ...
    def setPos(self, event):
        self.player.setPos(int(self.scl_time.get())*1000)
